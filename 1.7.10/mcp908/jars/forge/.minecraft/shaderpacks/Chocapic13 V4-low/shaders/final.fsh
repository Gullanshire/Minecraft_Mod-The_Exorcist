...
#define GODRAYS
	const float exposure = 1.2;			//godrays intensity 1.2 is default
	const float density = 0.15;			
	const int NUM_SAMPLES = 4;			//increase this for better quality at the cost of performance /3 is default
	const float grnoise = 0.0;		//amount of noise /0.0 is default
	
#define LENS_EFFECTS
#define LENS_STRENGTH 1.2
...
vec3 pattern[16] = vec3[16](	vec3(0.1,0.1,0.02),
								vec3(-0.12,0.07,0.02),
								vec3(-0.11,-0.13,0.02),
								vec3(0.1,-0.1,0.02),
...
void main() {
	...
	vec3 color = texture2D(composite, texcoord.st + fake_refract * 0.005).rgb;
...
*/
	...
	float avgdecay = 0.0;
            ...
            illuminationDecay = pow(max(1.0-sqrt(distx*distx+disty*disty),0.0),4.0);
	...
	color.rgb = mix(color.rgb,pow(sunlight,vec3(1.0/4.0)),(gr/NUM_SAMPLES)*exposure*length(pow(sunlight,vec3(1.0/4.0)))*(1.0 - rainStrength*0.85)*illuminationDecay*truepos/sqrt(3.0));
	}
	...
	for ( int i = 0; i < 60; i++) {
		sample = texture2D(composite, texcoord.xy + hex_offsets[i]*pcoc*vec2(1.0,aspectRatio));
		bcolor += sample.rgb;
	}
...
const int GL_EXP = 2048;
	//color.rgb = color.rgb*(1.0+(luma(color.rgb)-texture2D(composite,vec2(1.0)).a));
	vec3 blur = vec3(0.0);
	...
	if (fogMode == 0) fog = 1.0-clamp(exp(-ld(texture2D(depthtex2, texcoord.st).r)),0.0,1.0);
			...
			vec3 csample = vec3(0.0);
				...
				vec3 sample = texture2D(composite,texcoord.xy + coords*B_RAD*scale).rgb;
				csample += max(texture2D(composite,texcoord.xy + coords*B_RAD*scale).rgb-plum*0.75-B_TRESH,0.0) * (length(coords)+0.6)/2.0;
				blur += sample;
			}
...
float sunvisibility = min(texture2D(composite,vec2(0.0)).a*2.5,1.0) * (1.0-rainStrength*0.9) * fading * transition_fading;
...
/*
	if (texcoord.x < 0.1 && texcoord.y < 0.1) color.rgb = vec3(texture2D(composite,vec2(1.0)).a);
*/
color = clamp(color,0.0,1.0);
