...
const float STEPSIZE = 1.04;		//higher means lower quality
const float RANGE = 0.6;		//max ray length across the screen
...
vec4 ComputeReflection(vec3 delta) {
    ...
    vec4 color = vec4(texture2D(composite,texcoord.xy).rgb,0.0);
...
void main() {
    ...
    vec3 cameraSpacePosition = convertScreenSpaceToWorldSpace(texcoord.xy);
...
//Common variables
	...
	float iswater = texture2D(gaux2,texcoord.xy).r;
...
//REFRACTION
	...
	float waves = texture2D(gaux2, texcoord.st).g*2.0-1.0;
	...
	int samples = 1;
			...
			float redge = distance(texcoord.s, 0.5f);
			  ...
			  redge = 1.0f;
					...
					vec2 dir = vec2(waves*0.005, 0.005)/(pow(ld(depth),5.0)+0.3);
					for (int i = 0; i < 11; ++i) {
						
						
						rcoord = rcoord + dir;
						
						float thisdepth = texture2D(depthtex0, clamp(texcoord.st + rcoord, 0.001f, 0.999f)).x;
						...
						depthcheck2 = clamp(depthcheck * 70.0 - 69.0f, 0.0f, 1.0f);
							...
							reflectmask   = ((1.0 - texture2D(gaux2, clamp(texcoord.st + (rcoord*depthcheck), 0.001f, 0.999f)).r) * ((9.0 - samples)/9.0f));
							reflection  += 	(texture2D(composite, clamp(texcoord.st + (rcoord*depthcheck), 0.001f, 0.999f)).rgb * reflectmask) * ((9.0 - samples)/9.0f);
			...
			reflection = reflection*4.0;
						
			//Darken objects behind water
			color.rgb = mix(color.rgb,reflection,clamp(length(reflection),0.0,1.0));
...
for (int i = -3; i < 4; i++) {
	...
	ddiff -= -focal;
	bcolor += texture2D(composite,texcoord.xy + samplecoord).rgb*ddiff;
	mult += ddiff;
	}
	}
}
...
#ifdef BLOOM
float sweight = length(color.rgb)+(1.0-rainx)*0.2;
//exclude bright pixels
if (sweight < 0.7) {
vec3 csample = vec3(0.0);
for (int i=0; i < 16; i++) {
csample += max(texture2D(composite,texcoord.xy + offsets[i]*1.2).rgb-sweight,0.0);
}
color.rgb += csample/30.0;
}
...
/*
					for (int a = 0; a < 16; a++) {
							sunmask += 1.0f - texture2D(composite, lPos + offsets[a]*4.0).a;
						}
						sunmask /= 16.0;
*/
...
color.rgb = color.rgb / (color.rgb + 0.8) * 1.8;
