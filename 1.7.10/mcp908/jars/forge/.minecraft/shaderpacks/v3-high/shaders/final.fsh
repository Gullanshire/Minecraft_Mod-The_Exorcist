...
void main() {
	...
	vec3 color = texture2D(gaux3, texcoord.st).rgb;
	float land = texture2D(gaux3, texcoord.st).a;
...
//REFRACTION
	...
	int samples = 1;
			  ...
			  redge = 1.0f;
					...
					for (int i = 0; i < 11; ++i) {
						...
						depthcheck2 = clamp(depthcheck * 70.0 - 69.0f, 0.0f, 1.0f);
							...
							reflection  += 	(texture2D(gaux3, clamp(texcoord.st + (rcoord*depthcheck), 0.001f, 0.999f)).rgb * reflectmask) * ((9.0 - samples)/9.0f);
			...
			color = mix(color,reflection,clamp(length(reflection),0.0,1.0));
